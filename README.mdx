...
To get started, we recommend installing Prodo with `yarn add --dev @prodo-ai/snoopy-cli` or `npm install --save-dev @prodo-ai/snoopy-cli`.
...
```shell
$ <path to Prodo>/snoopy/node_modules/.bin/ts-node  <path to Prodo>/snoopy/packages/cli/src/index.ts start
```
...
We'd love your feedback! You can reach us at product@prodo.ai.
